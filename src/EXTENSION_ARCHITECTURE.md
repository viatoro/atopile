...
- Shared RPC/types: `src/ui/protocol`
- Python core runtime: `src/atopile/server`, `src/atopile/agent`
- Standalone layout tool: `src/atopile/layout_server` (not part of the extension runtime path)
...
- `panel-layout` now uses the same shared RPC websocket as the other panels.
- The standalone `src/atopile/layout_server` app remains available for direct
  browser debugging, but the extension does not mount or proxy it.
