...
Introspects the STDLIB_TYPES using the TypeGraph and returns
...
from atopile.data_models import StdLibChild, StdLibData, StdLibItem, StdLibItemType
...
def _get_item_type_for_stdlib(
    ...
    try:
        ...
        # Anything that can attach to a footprint is a component from
        # the user's perspective (BJT, MOSFET, etc.), not just pickables.
        if fabll.TypeNodeBoundTG.has_instance_of_type_has_trait(
            type_node, F.Footprints.can_attach_to_footprint
        ):
...
def _build_stdlib_items(max_depth: int | None = None) -> list[StdLibItem]:
    """Build StdLibItem list by introspecting STDLIB_TYPES via TypeGraph."""
    import faebryk.core.faebrykpy as fbrk
    import faebryk.core.graph as graph
    import faebryk.core.node as fabll
    ...
    for cls in STDLIB_TYPES:
        ...
        # The TG-based trait check can miss trait definitions; use the
        # Python-level check as an authoritative override.
        if item_type != StdLibItemType.TRAIT and fabll.Traits.is_trait_type(cls):
