...
`python -m atopile.generate_types`.
"""
...
from atopile.autolayout.models import AutolayoutState
...
class Log:
    ...
    # -------------------------------------------------------------------------
    # Base Pydantic models for log streaming queries
    # -------------------------------------------------------------------------
...
class ManufacturingArtifact(CamelModel):
    """A single file produced by a manufacturing build."""
...
class ModuleChild(CamelModel):
    ...
    src_loc: Optional[str] = None  # Source location as "file:line:col"
...
class UiPackageSyncProgress(CamelModel):
    """Progress of an in-flight package install/remove/sync operation."""
...
class UiPackageDetailState(CamelModel):
    ...
    sync_progress: UiPackageSyncProgress | None = None


class UiPartData(CamelModel):
    """Display-ready part data shared by the list and detail views."""
    ...
    installed: bool = False
    action: Literal["idle", "installing", "uninstalling", "converting"] = "idle"


class UiPartsSearchData(CamelModel):
    """Canonical parts panel state for the sidebar."""

    project_root: str | None = None
    query: str = ""
    installed_only: bool = False
    parts: list[UiPartData] = Field(default_factory=list)
    loading: bool = False
    error: str | None = None
    action_error: str | None = None
...
class UiStackupManufacturer(CamelModel):
    """Manufacturer information for the PCB stackup."""
...
class UiStackupLayer(CamelModel):
    """Single PCB stackup layer for the VS Code UI."""
...
class UiStackupData(CamelModel):
    """PCB stackup panel data for the VS Code UI store."""
    ...
    manufacturer: UiStackupManufacturer | None = None
    layers: list[UiStackupLayer] = Field(default_factory=list)
    layer_count: int = 0
    total_thickness_mm: float | None = None


class PinSignalType(StrEnum):
    LOGIC = "logic"
    SIGNAL = "signal"
    POWER = "power"
    NC = "nc"
...
class PinoutLead(CamelModel):
    lead_designator: str = ""
    pad_numbers: list[str] = Field(default_factory=list)
    net_name: str | None = None
    signal_type: PinSignalType
    interfaces: list[str] = Field(default_factory=list)
    is_connected: bool = False


class PinoutComponent(CamelModel):
    name: str = ""
    ato_address: str = ""
    designator: str = ""
    descriptor: str = ""
    ...
    leads: list[PinoutLead] = Field(default_factory=list)
    warnings: list[str] = Field(default_factory=list)


class UiPinoutData(CamelModel):
    """Pinout panel data for the VS Code UI store."""
...
class UiRecentBuildsData(CamelModel):
    """All recent builds across all projects."""
...
class UiAutolayoutCandidateData(CamelModel):
    """A single autolayout candidate for the UI."""

    candidate_id: str = ""
    label: str | None = None
    score: float | None = None
    # Backend-derived display fields — frontend should consume these
    # rather than re-deriving from `metadata`.
    routed_pct: int | None = None
    via_count: int | None = None
    metadata: dict[str, Any] = Field(default_factory=dict)
    files: dict[str, str] = Field(default_factory=dict)


class UiAutolayoutJobData(CamelModel):
    """A single autolayout job for the UI."""
    ...
    provider: str = ""
    job_type: str = ""
    state: AutolayoutState = AutolayoutState.BUILDING
    # Collapsed display state (idle / running / done / failed) — derived
    # by the backend so the frontend doesn't re-encode the state machine.
    ...
    # Best candidate by routed percentage — backend is the single
    # source of truth for "what to recommend".
    recommended_candidate_id: str | None = None
    layout_path: str | None = None
    candidates: list[UiAutolayoutCandidateData] = Field(default_factory=list)


class UiAutolayoutPreCheckItem(CamelModel):
    """A single readiness check displayed before submitting a job."""
...
class UiAutolayoutPreflightData(CamelModel):
    """Board preflight metrics for the UI."""

    board_area_mm2: float | None = None
    board_width_mm: float | None = None
    board_height_mm: float | None = None
    component_count: int = 0
    top_component_count: int = 0
    bottom_component_count: int = 0
    components_inside_board: int = 0
    components_outside_board: int = 0
    component_area_mm2: float | None = None
    pad_count: int = 0
    net_count: int = 0
    connection_count: int = 0
    placement_utilization: float | None = None
    top_only_utilization: float | None = None
    pad_density: float | None = None
...
class UiAutolayoutData(CamelModel):
    """Full autolayout panel state pushed to the sidebar."""

    loading: bool = False
    error: str | None = None
    submitting: bool = False
    job_type: str = "Placement"
    timeout_minutes: int = 1
    jobs: list[UiAutolayoutJobData] = Field(default_factory=list)
    preflight: UiAutolayoutPreflightData | None = None
    preflight_loading: bool = False
    preflight_error: str | None = None
    # Backend-computed readiness pre-checks — frontend just renders.
    placement_readiness: list[UiAutolayoutPreCheckItem] = Field(default_factory=list)
    routing_readiness: list[UiAutolayoutPreCheckItem] = Field(default_factory=list)
