...
OUT = ROOT / "src/ui/protocol/generated-types.ts"
...
@cache
def _schema_modules() -> tuple[
    types.ModuleType,
    types.ModuleType,
    type[BaseModel],
    dict[str, Any],
]:
    import atopile.data_models as data_models_module
    import atopile.server.ui.store as store_module
    from atopile.data_models import CamelModel
    from atopile.server.ui.store import STORE_SCHEMA

    return data_models_module, store_module, CamelModel, STORE_SCHEMA


def discover_camel_models() -> list[type[BaseModel]]:
    data_models_module, store_module, camel_model, _store_schema = _schema_modules()
    models: list[type[BaseModel]] = []
    for module in (data_models_module, store_module):
        for value in vars(module).values():
            if not isinstance(value, type):
                continue
            if not issubclass(value, camel_model) or value is camel_model:
                continue
...
def discover_enums(models: list[type[BaseModel]]) -> list[type[Enum]]:
...
def collect_enums(annotation: Any, enums: dict[str, type[Enum]]) -> None:
    ...
    if isinstance(annotation, type):
        if issubclass(annotation, Enum):
            ...
            return
        _, _, camel_model, _store_schema = _schema_modules()
        if issubclass(annotation, BaseModel) and not issubclass(
            annotation, camel_model
        ):
            raise RuntimeError(
...
def render_store_keys() -> list[str]:
    _, _, _camel_model, store_schema = _schema_modules()
...
def annotation_to_ts(annotation: Any) -> str:
    ...
    if isinstance(annotation, type):
        if issubclass(annotation, BaseModel):
            _, _, camel_model, _store_schema = _schema_modules()
            if not issubclass(annotation, camel_model):
                raise RuntimeError(
                    "CamelModel field references non-CamelModel"
                    f" schema: {annotation.__name__}"
                )
