...
import { CenteredSpinner, EmptyState, Button, Input } from "../common/components";
import { fileStyle } from "../common/utils/fileIcons";
import { WebviewRpcClient, rpcClient } from "../common/webviewRpcClient";
import { createWebviewLogger } from "../common/logger";
import { isWebIde } from "../common/render";
import type { FileNode } from "../../protocol/generated-types";
import {
  joinPath,
  relativeToProject,
  parentRelativePath,
...
function contextMenuItems(target: ContextMenuTarget): { action: ExplorerAction; label: string }[] {
  const terminal: { action: ExplorerAction; label: string }[] = isWebIde
    ? []
  ...
  if (target.isRoot) {
    return [
      ...
      { action: "reveal", label: "Show in Explorer" },
      ...terminal,
    ];
  }
  if (target.isFolder) {
...
function TreeNode({
  ...
  const rowTarget: ContextMenuTarget = {
    ...
    isFolder,
    isRoot: false,
    directoryRelativePath: isFolder ? path : parentRelativePath(path),
  };
  ...
  const { icon: Icon, className: iconClass, imageSrc } = isFolder
    ? {
    ...
    <div className="tree-node">
      ...
      >
        ...
        <span className={`tree-row-icon ${iconClass}`}>
          {imageSrc ? (
            <img src={imageSrc} alt="" className="tree-row-icon-image" />
          ) : Icon ? (
            <Icon size={14} />
          ) : null}
        </span>
...
export function FilesPanel() {
  ...
  const isLoading = Boolean(projectRoot) && projectFilesState.loading;
  const projectFiles = projectFilesState.files;
    ...
    async (action: ExplorerAction, target: ContextMenuTarget, value?: string) => {
      ...
      switch (action) {
        ...
        case "new-file": {
          ...
          rpcClient?.sendAction("createFile", {
            projectRoot,
            parentRelativePath: target.directoryRelativePath,
            name,
          });
          logger.info(
            `createFile requested dir=${target.directoryRelativePath} name=${name}`,
          );
          return;
        }
        ...
        case "new-folder": {
          const name = value?.trim() ?? "";
          rpcClient?.sendAction("createFolder", {
            projectRoot,
            parentRelativePath: target.directoryRelativePath,
            name,
          });
          logger.info(
            `createFolder requested dir=${target.directoryRelativePath} name=${name}`,
          );
          return;
        }
        ...
        case "rename": {
          ...
          if (newName === basename(target.fullPath)) {
            return;
          }
          ...
          logger.info(
            `renamePath requested target=${target.relativePath} next=${newName}`,
          );
          return;
        }
        ...
        case "duplicate": {
          ...
          logger.info(`duplicatePath requested target=${target.relativePath}`);
          return;
        }

        case "delete":
          rpcClient?.sendAction("deletePath", {
            projectRoot,
            relativePath: target.relativePath,
          });
          logger.info(`deletePath completed target=${target.relativePath}`);
          return;
