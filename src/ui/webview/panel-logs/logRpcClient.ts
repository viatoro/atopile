...
import type { SocketLike } from "../../protocol/rpcTransport";
import type {
  UiBuildLogRequest,
  UiLogEntry,
  UiLogsErrorMessage,
...
type RawLogMessage =
  | UiLogsErrorMessage
  | {
      type: "logs_stream" | "test_logs_stream";
      buildId?: string;
      build_id?: string;
      testRunId?: string;
      test_run_id?: string;
...
export function createLogClient(
  ...
  return new LogRpcClient({
    ...
    startStream(request) {
      transport.send(JSON.stringify(serializeWebSocketRequest(request)));
    },
    stopStream() {
      if (connected) {
