...
import { ansiConverter } from '../common/utils';
  ...
  if (!isContainerType || depth >= maxDepth) {
    ...
    return (
      <span className="cv-primitive">
        ...
        {value.truncated && <span className="cv-truncated">&hellip;</span>}
      </span>
    );
  }
          ...
          return (
            <div key={originalIndex} className={`si-frame ${isExpanded ? 'expanded' : ''}`}>
              ...
              >
                ...
                <span
                  className="si-location si-location-link"
                  role="link"
                  title={`${frame.filename}:${frame.lineno}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    void rpcClient?.requestAction("vscode.openFile", {
                      path: frame.filename,
                      line: frame.lineno,
                      column: frame.colno ?? undefined,
                    });
                  }}
                >
