...
function stageStatusIcon(status: BuildStage["status"]) {
  if (status === "running") return <Spinner size={12} />;
  const Icon = STATUS_ICONS[status] ?? AlertCircle;
  return <Icon size={12} className={`stage-icon-${status}`} />;
}
...
export function BuildQueueItem({
  ...
  const hasFailedStage = build.stages.some((stage) => stage.status === "failed");
  const buildCounter = getBuildCounter(build.buildId);
