...
import { dirname, formatPath, relativeTargetRoot, samePath } from "../../protocol/paths";
...
const projectFuseOptions = {
  keys: [
    ...
    { name: "identifier", weight: 0.6 },
    { name: "targets.name", weight: 0.5 },
    { name: "root", weight: 0.3 },
    { name: "targets.entry", weight: 0.3 },
  ],
  threshold: 0.4,
  includeMatches: true,
  ignoreLocation: true,
...
function ProjectSelector({
  ...
  const fuse = useMemo(() => new Fuse(projects, projectFuseOptions), [projects]);
  ...
  return (
    <div className="project-combobox" ref={comboboxRef}>
      <div
        ...
        onClick={(e) => {
          if (!(e.target as HTMLElement).closest(".combobox-toggle")) {
            inputRef.current?.focus();
          }
        }}
      >
        <FolderOpen size={12} className="combobox-icon" />
        <input
          ref={inputRef}
          type="text"
          ...
          <div className="combobox-list" role="listbox">
            {filteredProjects.length === 0 ? (
              ...
              filteredProjects.map((project, index) => {
                const isActive = samePath(project.root, activeProject?.root);
                ...
                return (
                  ...
                  >
                    {project.error ? (
                      <AlertCircle size={12} className="option-icon-error" />
                    ) : (
          ...
          <div className="target-combobox-list" role="listbox">
            {targets.map((target, index) => {
              const isActive = Boolean(
                ...
                && target.entry === activeTarget.entry
                && samePath(target.root, activeTarget.root),
              );
              const isHighlighted = index === highlightedIndex;
...
function NewTargetForm({
  ...
  const entryCheckMatches =
    !!entry.trim() &&
    samePath(entryCheck.projectRoot, projectRoot) &&
...
export function ProjectTargetSelector({
  ...
  const activeProject = useMemo(
    () => projects.find((project) => samePath(project.root, selectedProjectRoot)) ?? null,
  ...
  const defaultParentDirectory = useMemo(() => {
    if (activeProject) {
      return dirname(activeProject.root) || activeProject.root;
    }
    const firstProject = projects[0];
    if (!firstProject) return "";
    return dirname(firstProject.root) || firstProject.root;
      ...
      {showNewProjectForm ? (
        <NewProjectForm
          ...
          onSubmit={async (data) => {
            ...
            try {
              await rpcClient?.requestAction("createProject", { ...data });
        ...
        <div className="build-targets">
          ...
          <div className="target-actions">
              <Tooltip>
                <TooltipTrigger>
                  <button
                    className="icon-action-btn target-action-add"
                ...
                <TooltipContent side="bottom">Create new build</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger>
                  <button
                    className="icon-action-btn danger target-action-delete"
                    onClick={async () => {
                    ...
                    disabled={
                      ...
                      || !activeTarget
                      || isDeletingTarget
                      || !samePath(activeTarget.root, activeProject.root)
                    }
                  >
                    <X size={13} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom">Delete selected build</TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger>
                  <button
                    className="icon-action-btn target-action-edit"
                    onClick={() => setShowEditTargetForm(true)}
                    disabled={!activeProject || !activeTarget || !samePath(activeTarget.root, activeProject.root)}
                  >
                    <Pencil size={13} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom">Edit selected build</TooltipContent>
              </Tooltip>
              <button
                className="build-target-btn target-action-build"
                onClick={onBuild}
                disabled={buildDisabled || isBuilding}
              >
                {isBuilding ? <Spinner size={13} /> : <Hammer size={13} />}
                <span className="build-target-btn-label">
