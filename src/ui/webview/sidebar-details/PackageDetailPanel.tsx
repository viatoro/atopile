...
export function PackageDetailPanel({
  ...
  const [selectedVersion, setSelectedVersion] = useState(latestAvailableVersion)
  ...
  // Default to latest when versions load, unless user already picked one
  useEffect(() => {
    if (sortedVersions.length > 0 && !userPickedVersion) {
      setSelectedVersion(sortedVersions[0].version)
    }
  }, [sortedVersions, userPickedVersion])
