...
import type { LayoutDecorations, PadDecoration, FootprintDecoration } from "./editor";
...
const PAD_HIGHLIGHT_INFLATE = 0.12;
const PAD_HIGHLIGHT_STROKE_FACTOR = 0.34;
const PAD_HIGHLIGHT_MIN_STROKE = 0.12;
const PAD_LOOP_ARC_SEGMENTS = 18;
const PAD_OFFSET_MITER_LIMIT = 4;
...
function padHighlightLayerName(layerName: string): string {
    return `pad-highlight:${layerName}`;
}

/** Approximate a 3-point arc with line segments */
export function arcToPoints(start: Point2, mid: Point2, end: Point2, segments = 32): Vec2[] {
...
function drawFootprintSelectionBox(
    ...
    if (fillAlpha > 0) {
        layer.geometry.add_polygon(corners.slice(0, 4), r, g, b, fillAlpha);
    }
    layer.geometry.add_polyline(corners, strokeWidth, r, g, b, strokeAlpha);
}

export function buildLayerMap(model: RenderModel): Map<string, LayerModel> {
...
export function paintObjects(
    ...
    const padsByLayer = new Map<string, Array<{ at: Point3; p: PadModel; ownerId: string | null; style?: PadDecoration }>>();
    const highlightedPadsByLayer = new Map<string, Array<{ at: Point3; p: PadModel; ownerId: string | null; style?: PadDecoration }>>();
        ...
        if (showPads) {
            for (const p of fp.pads) {
                ...
                for (const l of p.layers) {
                    ...
                    const style = decorations?.pads?.get(`${ownerId ?? footprintOwnerId(fp, -1)}:pad:${p.name}`);
                    arr.push({
                        at: fp.at,
                        p,
                        ownerId,
                    ...
                    if (style?.highlighted) {
                        let highlightArr = highlightedPadsByLayer.get(l);
                        if (!highlightArr) {
                            highlightArr = [];
                            highlightedPadsByLayer.set(l, highlightArr);
                        }
                        highlightArr.push({ at: fp.at, p, ownerId, style });
                    }
                }
                if (p.hole) {
    ...
    for (const ln of sortedNames) {
        ...
        if (tint) {
            [r, g, b, a] = tintBlend(r, g, b, a, tint);
        }
        ...
        if (pads) for (const { at, p, ownerId, style } of pads) paintPad(layer, at, p, ln, layerById, ownerId, style);
    ...
    for (const layerName of [...highlightedPadsByLayer.keys()].sort(
        (a, b) => layerPaintOrder(a, layerById) - layerPaintOrder(b, layerById),
    )) {
        const highlightLayer = renderer.get_layer(padHighlightLayerName(layerName));
        const pads = highlightedPadsByLayer.get(layerName);
        if (!pads) continue;
        for (const { at, p, ownerId, style } of pads) {
            paintPadHighlight(highlightLayer, at, p, layerName, layerById, ownerId, style);
        }
    }
}

/** Paint footprint-level decorations (colored outlines/fills around footprint bounding boxes). */
export function paintFootprintDecorations(
    renderer: Renderer,
    footprints: FootprintModel[],
    decorations: Map<string, FootprintDecoration>,
): void {
    const regularLayer = renderer.get_layer("fp-decoration");
    const highlightLayer = renderer.get_layer("fp-decoration-highlight");

    for (let i = 0; i < footprints.length; i++) {
        const fp = footprints[i]!;
        if (!fp.uuid) continue;
        const dec = decorations.get(fp.uuid);
        if (!dec) continue;

        const c = dec.highlighted
            ? (dec.highlightColor ?? dec.color ?? [1, 1, 1, 0.6])
            : (dec.color ?? [1, 1, 1, 0.3]);
        const [r, g, b, a] = c;
        const layer = dec.highlighted ? highlightLayer : regularLayer;
        const strokeWidth = dec.highlighted ? 0.1 : 0.08;
        const grow = dec.highlighted ? 0.14 : 0.1;
        const fillAlpha = dec.highlighted ? a * 0.2 : a * 0.15;

        drawFootprintSelectionBox(layer, fp, strokeWidth, a, grow, fillAlpha, r, g, b);
    }
}
...
export function paintStaticBoard(
    ...
    if (decorations?.footprints && decorations.footprints.size > 0) {
        paintFootprintDecorations(renderer, model.footprints, decorations.footprints);
    }
...
export function paintAll(
    ...
    paintStaticBoard(renderer, model, hiddenLayers, undefined, decorations);
}

export function paintBoardEdges(renderer: Renderer, model: RenderModel, layerById: Map<string, LayerModel>) {
...
function paintPadHole(
    ...
    paintFilledLoops(layer, buildHoleLoops(fpAt, pad, hole), r, g, b, a, ownerId);
}
...
function closeLoop(points: Vec2[]): Vec2[] {
    return points.length > 0 ? [...points, points[0]!.copy()] : points;
}

function lineIntersection(originA: Vec2, directionA: Vec2, originB: Vec2, directionB: Vec2): Vec2 | null {
    const cross = directionA.x * directionB.y - directionA.y * directionB.x;
    if (Math.abs(cross) < 1e-6) {
        return null;
    }
    const delta = originB.sub(originA);
    const t = (delta.x * directionB.y - delta.y * directionB.x) / cross;
    return originA.add(directionA.multiply(t));
}
...
function pointInPolygon(point: Vec2, polygon: Vec2[]): boolean {
    ...
    for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
        ...
        const edgeDy = previous.y - current.y;
        const intersectionX = ((previous.x - current.x) * (point.y - current.y)) / (Math.abs(edgeDy) < 1e-9 ? 1e-9 : edgeDy) + current.x;
...
function buildPadOuterLoops(fpAt: Point3, pad: PadModel): Vec2[][] {
    const hw = Math.max(0, pad.size.w / 2);
    const hh = Math.max(0, pad.size.h / 2);

    if (pad.shape === "custom" && pad.primitives && pad.primitives.length > 0) {
        return pad.primitives
            .filter((primitive) => primitive.points.length >= 3)
            .map((primitive) => primitive.points.map(
                (point) => padTransform(fpAt, pad.at, point.x, point.y),
            ));
    }

    const radius = pad.shape === "rect"
        ? 0
        : pad.shape === "roundrect"
            ? Math.min(hw, hh) * Math.max(0, Math.min(1, pad.roundrect_rratio ?? 0))
            : Math.min(hw, hh);
    if (radius <= 0) {
        return [[
    ...
    const centers = [
        { x: hw - clampedRadius, y: hh - clampedRadius, start: 0, end: Math.PI / 2 },
        { x: -hw + clampedRadius, y: hh - clampedRadius, start: Math.PI / 2, end: Math.PI },
        { x: -hw + clampedRadius, y: -hh + clampedRadius, start: Math.PI, end: 3 * Math.PI / 2 },
        { x: hw - clampedRadius, y: -hh + clampedRadius, start: 3 * Math.PI / 2, end: 2 * Math.PI },
    ];
    const points: Vec2[] = [];
    for (const corner of centers) {
        for (let i = 0; i <= PAD_LOOP_ARC_SEGMENTS; i++) {
            if (points.length > 0 && i === 0) continue;
            const angle = corner.start + ((corner.end - corner.start) * i / PAD_LOOP_ARC_SEGMENTS);
            points.push(
                padTransform(
                    fpAt,
                    pad.at,
                    corner.x + clampedRadius * Math.cos(angle),
                    corner.y + clampedRadius * Math.sin(angle),
                ),
            );
        }
    }
...
function buildHoleLoops(fpAt: Point3, pad: PadModel, hole: HoleModel): Vec2[][] {
    ...
    const centers = [
        { x: hw - radius, y: hh - radius, start: 0, end: Math.PI / 2 },
        { x: -hw + radius, y: hh - radius, start: Math.PI / 2, end: Math.PI },
        { x: -hw + radius, y: -hh + radius, start: Math.PI, end: 3 * Math.PI / 2 },
        { x: hw - radius, y: -hh + radius, start: 3 * Math.PI / 2, end: 2 * Math.PI },
    ];
    const points: Vec2[] = [];
    for (const corner of centers) {
        for (let i = 0; i <= PAD_LOOP_ARC_SEGMENTS; i++) {
...
function offsetClosedLoop(points: Vec2[], offset: number): Vec2[] {
    if (points.length < 3 || Math.abs(offset) < 1e-6) {
        return points.map((point) => point.copy());
    }

    const orientation = signedArea(points);
    const outwardScale = orientation >= 0 ? -1 : 1;

    return points.map((point, index) => {
        const previous = points[(index - 1 + points.length) % points.length]!;
        const next = points[(index + 1) % points.length]!;
        const incoming = point.sub(previous).normalize();
        const outgoing = next.sub(point).normalize();
        const normalIn = incoming.normal.multiply(outwardScale);
        const normalOut = outgoing.normal.multiply(outwardScale);
        const offsetInOrigin = point.add(normalIn.multiply(offset));
        const offsetOutOrigin = point.add(normalOut.multiply(offset));
        const intersection = lineIntersection(offsetInOrigin, incoming, offsetOutOrigin, outgoing);

        if (intersection) {
            const delta = intersection.sub(point);
            if (delta.magnitude <= Math.abs(offset) * PAD_OFFSET_MITER_LIMIT) {
                return intersection;
            }
        }

        const fallbackNormal = normalIn.add(normalOut).normalize();
        if (fallbackNormal.magnitude > 0) {
            return point.add(fallbackNormal.multiply(offset));
        }

        return offsetInOrigin;
    });
}

function paintFilledLoops(
    layer: RenderLayer,
    outerLoops: Vec2[][],
    r: number,
    g: number,
    ...
    const validHoles = holeLoops.filter((loop) => loop.length >= 3);
    for (const outerLoop of outerLoops) {
        if (outerLoop.length < 3) {
            continue;
        }
        const containedHoles = validHoles.filter(
            (holeLoop) => pointInPolygon(holeLoop[0]!, outerLoop),
        );
        if (containedHoles.length > 0) {
            layer.geometry.add_polygon_with_holes(outerLoop, containedHoles, r, g, b, a, ownerId);
            continue;
        }
        layer.geometry.add_polygon(outerLoop, r, g, b, a, ownerId);
    }
}

function paintLoopOutlines(
    layer: RenderLayer,
    loops: Vec2[][],
    strokeWidth: number,
    r: number,
    ...
    for (const loop of loops) {
        ...
        const outline = offsetClosedLoop(loop, offset);
        layer.geometry.add_polyline(closeLoop(outline), strokeWidth, r, g, b, a, ownerId);
    }
}
...
function paintPad(
    ...
    const defaultPadColor: Color = [lr, lg, lb, Math.max(la, 0.78)];
    const [cr, cg, cb, ca] = style?.color ?? defaultPadColor;
    const hw = pad.size.w / 2;
    const hh = pad.size.h / 2;
    const outerLoops = buildPadOuterLoops(fpAt, pad);

    if (style?.outlined) {
        const strokeWidth = Math.max(Math.min(hw, hh) * 0.22, 0.06);
        paintLoopOutlines(layer, outerLoops, strokeWidth, cr, cg, cb, ca, ownerId, -strokeWidth / 2);
        return;
    }
    ...
    if (renderAsAnnularRing) {
        const holeLoops = hole ? buildHoleLoops(fpAt, pad, hole) : [];
        if (holeLoops.length > 0) {
            paintFilledLoops(layer, outerLoops, cr, cg, cb, ca, ownerId, holeLoops);
            return;
        }
    }

    paintFilledLoops(layer, outerLoops, cr, cg, cb, ca, ownerId);

    // Pad holes are rendered in dedicated depth layers (see paintFootprint).
}

function paintPadHighlight(
    layer: RenderLayer,
    fpAt: Point3,
    pad: PadModel,
    layerName: string,
    ...
    if (!style?.highlighted || pad.layers.length === 0) {
        return;
    }
    const [lr, lg, lb, la] = getLayerColor(layerName, layerById);
    const defaultPadColor: Color = [lr, lg, lb, Math.max(la, 0.78)];
    const [cr, cg, cb, ca] = style.color ?? defaultPadColor;
    const [hr, hg, hb, ha] = style.highlightColor ?? [cr, cg, cb, Math.max(ca, 0.98)];
    const strokeWidth = Math.max(
        Math.min(pad.size.w / 2, pad.size.h / 2) * PAD_HIGHLIGHT_STROKE_FACTOR,
        PAD_HIGHLIGHT_MIN_STROKE,
    );
    paintLoopOutlines(
        layer,
        buildPadOuterLoops(fpAt, pad),
        strokeWidth,
        hr,
        hg,
        hb,
        ha,
        ownerId,
        PAD_HIGHLIGHT_INFLATE + strokeWidth / 2,
    );
}
