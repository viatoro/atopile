...
import { footprintBBox, hitTestFootprintsInBox, hitTestPads } from "./hit-test";
import type { LayoutTransport } from "./layout_client";
...
import type { LayoutClientLogger } from "./layout_client";
import type { Color } from "./colors";
import type { ActionCommand, DrawingModel, FootprintModel, LayerModel, RenderDelta, RenderModel, ZoneModel } from "./types";
...
export interface FootprintDecoration {
    color?: Color;
    highlightColor?: Color;
    highlighted?: boolean;
}

export interface LayoutDecorations {
    pads?: Map<string, PadDecoration>;
    footprints?: Map<string, FootprintDecoration>;
}

export function getLayoutFootprintId(footprint: FootprintModel, fallbackIndex: number): string {
    return footprint.uuid ? `fp:${footprint.uuid}` : `fp_idx:${fallbackIndex}`;
}

export function getLayoutPadId(footprintId: string, padName: string): string {
    return `${footprintId}:pad:${padName}`;
}
...
export class Editor {
    ...
    private client: LayoutTransport;
    ...
    private decorations: LayoutDecorations | null = null;
    private onPadHoverCallback: ((padId: string | null) => void) | null = null;
    private onSelectionChangeCallback: ((indices: number[]) => void) | null = null;
    private onFootprintHoverCallback: ((fpUuid: string | null) => void) | null = null;
    private onFootprintClickCallback: ((fpUuid: string | null) => void) | null = null;
    private suppressBuiltinSelection = false;
    private designatorToUuid = new Map<string, string>();
    private hoveredPadId: string | null = null;
    ...
    private readonly logger: LayoutClientLogger | undefined;
    ...
    constructor(canvas: HTMLCanvasElement, client: LayoutTransport, options: EditorOptions = {}) {
        this.canvas = canvas;
    ...
    private paintStatic(skipped?: DragSelection) {
        ...
        paintStaticBoard(this.renderer, this.model, geometryHidden, skipped, this.decorations ?? undefined);
    ...
    private paintDynamic() {
        ...
        if (!this.suppressBuiltinSelection) {
            if (
    ...
    private rebuildGroupIndex() {
        if (!this.model) return;
        const index = buildGroupIndex(this.model, this.logger);
    ...
    private rebuildAfterObjectTypeVisibilityChange() {
        ...
        if (this.isDragging) {
            ...
            paintDraggedSelection(this.renderer, this.model, draggedSelection, this.getLayerMap(), this.hiddenLayers, this.decorations ?? undefined);
    ...
    private updateHoverGroup(worldPos: Vec2) {
        ...
        const prevHoverFp = this.hoveredFpIndex;
        this.hoveredGroupId = nextHoverId;
        this.hoveredFpIndex = nextHoverFp;
        this.repaintWithSelection();
        if (this.onFootprintHoverCallback && prevHoverFp !== nextHoverFp && this.model) {
            const uuid = nextHoverFp >= 0 ? (this.model.footprints[nextHoverFp]?.uuid ?? null) : null;
            this.onFootprintHoverCallback(uuid);
        }
    }
    ...
    private setupMouseHandlers() {
        this.canvas.addEventListener("mousedown", (e: MouseEvent) => {
            ...
            if (this.readOnly) {
                // Read-only mode: allow click-to-select but no drag/box-select
                let hitIdx = -1;
                ...
                if (this.onFootprintClickCallback) {
                    const uuid = hitIdx >= 0 ? (this.model.footprints[hitIdx]?.uuid ?? null) : null;
                    this.onFootprintClickCallback(uuid);
                }
                return;
            }
        ...
        this.canvas.addEventListener("dblclick", (e: MouseEvent) => {
            ...
            this.repaintWithSelection();
            if (this.readOnly) this.fireSelectionChange();
        }, { signal: this.eventAbortController.signal });
        ...
        this.canvas.addEventListener("mousemove", (e: MouseEvent) => {
            ...
            const worldPos = this.camera.screen_to_world(this.lastMouseScreen);
            if (this.readOnly) {
                this.updateHoveredPad(worldPos);
                if (this.model) {
        ...
        window.addEventListener("mouseup", async (e: MouseEvent) => {
            ...
            if (!Number.isFinite(dx) || !Number.isFinite(dy)) {
                this.logger?.warn?.(`Ignoring drag move with invalid delta dx=${String(dx)} dy=${String(dy)}`);
                this.isDragging = false;
                this.restorePostDragRendering();
                this.clearDragState();
                this.repaintWithSelection();
            ...
            if (movePromise) {
                void movePromise.then(async (ok) => {
                    this.restorePostDragRendering();
                    if (!ok) {
                        await this.fetchAndPaint();
                        this.repaintWithSelection();
    ...
    private updateHoveredPad(worldPos: Vec2) {
        if (!this.model) {
            this.setHoveredPad(null);
            return;
        }
        for (let i = 0; i < this.model.footprints.length; i++) {
            const footprint = this.model.footprints[i]!;
            const padIdx = hitTestPads(worldPos, footprint);
            if (padIdx >= 0) {
                this.setHoveredPad(
                    getLayoutPadId(getLayoutFootprintId(footprint, i), footprint.pads[padIdx]!.name),
                );
                return;
            }
        }
    ...
    private setHoveredPad(padId: string | null) {
        if (this.hoveredPadId === padId) {
            return;
        }
        this.hoveredPadId = padId;
        this.onPadHoverCallback?.(padId);
    }
    ...
    private async executeAction(
        ...
        try {
            ...
            if (data.status === "error") {
                this.logger?.warn?.(
                    `Action ${action.command} failed (${data.code}): ${data.message ?? "unknown error"}`,
                );
            } else {
                ok = true;
            }
        } catch (err) {
            this.logger?.error?.(`Failed to execute action ${action.command}: ${formatErrorDetail(err)}`);
        } finally {
    ...
    setDecorations(decorations: LayoutDecorations | null) {
    ...
    setRenderModel(model: RenderModel, fitToView = false) {
        this.applyModel(model, fitToView);
    }

    setOnPadHover(cb: ((padId: string | null) => void) | null) {
        this.onPadHoverCallback = cb;
        if (!cb) {
            this.hoveredPadId = null;
        }
    }

    setOnSelectionChange(cb: ((indices: number[]) => void) | null) {
        this.onSelectionChangeCallback = cb;
    }

    setSuppressBuiltinSelection(suppress: boolean) {
        this.suppressBuiltinSelection = suppress;
        this.repaintWithSelection();
    }

    setOnFootprintHover(cb: ((fpUuid: string | null) => void) | null) {
        this.onFootprintHoverCallback = cb;
    }

    setOnFootprintClick(cb: ((fpUuid: string | null) => void) | null) {
        this.onFootprintClickCallback = cb;
    }

    private rebuildDesignatorIndex() {
        this.designatorToUuid.clear();
        if (!this.model) return;
        for (const fp of this.model.footprints) {
            if (fp.reference && fp.uuid) {
                this.designatorToUuid.set(fp.reference, fp.uuid);
            }
        }
    }

    /** Resolve a designator (e.g. "R1") to its footprint UUID, or null. */
    getFootprintUuidByDesignator(designator: string): string | null {
        return this.designatorToUuid.get(designator) ?? null;
    }

    selectFootprintsByIndices(indices: number[]) {
        if (!this.model) return;
        this.setMultiSelection(indices);
        this.repaintWithSelection();
    }

    getSelectedFootprintIndices(): number[] {
        if (this.selectionMode === "single" && this.selectedFpIndex >= 0) {
            return [this.selectedFpIndex];
        }
    ...
    private fireSelectionChange() {
        if (this.onSelectionChangeCallback) {
            this.onSelectionChangeCallback(this.getSelectedFootprintIndices());
        }
    }
    ...
    private onRenderFrame() {
        ...
        const viewport = this.getCanvasViewportMetrics();
        this.camera.viewport_size = new Vec2(viewport.width, viewport.height);
        this.renderer.updateGrid(this.camera.bbox, 1.0, this.camera.zoom);
