...
export function buildGroupIndex(
    ...
    for (let i = 0; i < model.footprint_groups.length; i++) {
        ...
        if (rawGraphicUuids.length > 0 && graphicMemberUuids.length === 0) {
            logger?.warn?.(
                `Group has graphic_member_uuids but none matched drawingIndexByUuid group=${group.uuid || group.name || "unknown"} drawingCount=${model.drawings.length} drawingsWithUuid=${[...Array(model.drawings.length).keys()].filter(i => model.drawings[i]?.uuid).length} rawGraphicUuids=${rawGraphicUuids.join(",")}`,
            );
        }
