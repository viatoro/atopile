...
import { Zap, Package, Star, Trash2, Mail, Calendar, Globe, Settings, FileText, Copy, Search, Filter } from 'lucide-react'
...
import { SidebarDockPanel, SidebarDockHeader } from './SidebarDockPanel'
...
export function ComponentShowcase() {
  ...
  const [selectedVersion, setSelectedVersion] = useState('1.2.0')
    ...
    <div className="showcase">
      ...
      <section className="showcase-section">
        ...
        <p style={{ color: 'var(--text-secondary)', marginBottom: 'var(--spacing-md)' }}>
          Responsive bar that collapses items into a dropdown when space is limited. Resize the container to see overflow.
        </p>
        <div style={{ maxWidth: 320, border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 'var(--spacing-sm)', resize: 'horizontal', overflow: 'auto' }}>
          <OverflowBar
            items={[
              { key: 'search', icon: <Search size={14} />, label: 'Search', onClick: () => {} },
              { key: 'filter', icon: <Filter size={14} />, label: 'Filter', onClick: () => {} },
              { key: 'copy', icon: <Copy size={14} />, label: 'Copy', onClick: () => {} },
              { key: 'settings', icon: <Settings size={14} />, label: 'Settings', onClick: () => {} },
              { key: 'docs', icon: <FileText size={14} />, label: 'Docs', onClick: () => {} },
              { key: 'star', icon: <Star size={14} />, label: 'Star', onClick: () => {} },
            ]}
            minItemWidth={72}
            renderItem={(item) => (
      ...
      <section className="showcase-section">
        <h2>CenteredSpinner</h2>
        <div className="showcase-card" style={{ height: 100 }}>
      ...
      <section className="showcase-section">
        <h2>CopyableCodeBlock</h2>
        <div className="showcase-col" style={{ maxWidth: 400 }}>
          <CopyableCodeBlock
            label="Import"
            code={'import Resistor from "generics/Resistor.ato"'}
            highlightAto
          />
          <CopyableCodeBlock
            label="Usage"
            code={'r1 = new Resistor\nr1.value = 10kohm +/- 5%'}
            highlightAto
          />
        </div>
      </section>
        ...
        <div className="showcase-card">
          <JsonView
            value={{
              ...
              config: { autoRoute: true, layers: 4 },
            }}
            defaultOpen
          />
        </div>
      ...
      <section className="showcase-section">
        ...
        <div style={{ maxWidth: 400 }}>
          ...
          />
          <div className="showcase-card" style={{ marginTop: 'var(--spacing-sm)' }}>
      ...
      <section className="showcase-section">
        <h2>TypeIcon</h2>
        <div className="showcase-row">
          {(['interface', 'module', 'component', 'parameter', 'trait'] as const).map((t) => (
            <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--spacing-xs)' }}>
              {typeIcon(t, 16)} {t}
            </span>
          ))}
        </div>
      ...
      <section className="showcase-section">
        <h2>SidebarDockPanel</h2>
        <div style={{ maxWidth: 400 }}>
          <SidebarDockPanel collapsed={dockCollapsed} height={150}>
            <SidebarDockHeader
              title="Components"
              badge={<Badge variant="secondary">12</Badge>}
              collapsed={dockCollapsed}
              onToggleCollapsed={() => setDockCollapsed(!dockCollapsed)}
              actions={<Button variant="ghost" size="icon" aria-label="Settings"><Settings size={14} /></Button>}
            />
            {!dockCollapsed && (
      ...
      <section className="showcase-section">
        <h2>SidebarSubpanel</h2>
        <div style={{ maxWidth: 400 }}>
          <SidebarSubpanel>
            <div style={{ padding: 'var(--spacing-sm)', color: 'var(--text-secondary)' }}>
              A simple wrapper for sidebar panel content sections.
            </div>
          </SidebarSubpanel>
        </div>
      </section>
