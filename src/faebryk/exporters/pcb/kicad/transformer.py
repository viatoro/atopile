...
class PCB_Transformer:
    ...
    def map_footprints(
        ...
        for module in fabll.Traits.get_implementor_objects(
            ...
            atopile_addr = module.get_full_name(include_root=False)
    ...
    def set_designator_positions(
        ...
        center_when_possible: bool = False,
    ):
    ...
    def _fp_common_fields_dict(
        ...
        common_fields = set(kicad.footprint.Footprint.__field_names__()) & set(
            kicad.pcb.Footprint.__field_names__()
        )
        # Copy only the schema overlap so library-only metadata like
        # `version/generator` and PCB-only placement data stay isolated.
        return {field: getattr(lib_footprint, field) for field in common_fields}
